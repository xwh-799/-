  ...
  <div>
    ...
    <div class="main">
      <div class="py-container">
        ...
        <!--selector-->
        <SearchSelector @trademarkInfo="trademarkInfo" @attrInfos="attrInfos"/>
        ...
        <div class="details clearfix">
              ...
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click.prevent="changeOrder('1')">
                  <a>综合<span v-show="isOne" class="iconfont"
                               :class="{'icon-todown':isDown,'icon-up':isUp}"></span></a>
                </li>
          ...
          <div class="goods-list">
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="good in $search.goodsList" :key="good.id">
                <div class="list-wrap">
                  <div class="p-img">
          ...
          <div class="fr page">
            <PaginationM :pageNo="searchParams.pageNo" :pageSize="searchParams.pageSize"
                         :total="$search.searchList.total" :continues="5"
                         @getPageNo="getPageNo"
            ></PaginationM>
          </div>
        </div>
...
import {onBeforeMount, watch, onMounted, computed, getCurrentInstance, ref} from "vue";
import type {AttrsList, SearchParams, TrademarkList} from '@/interface/ResultType'
...
const removeCategoryName = (num: number) => {
  if (num === 1) {
    ...
    searchParams.value.trademark = undefined
    getData(searchParams.value)
...
const getData = (params: {} = {}) => {
  ...
  $search.getSearchList(params)
}

const changeOrder = (num: string) => {
  let originOrder = searchParams.value.order
  let originFlag = searchParams.value.order?.split(':')[0]
...
const trademarkInfo = (trademark: TrademarkList) => {
  searchParams.value.trademark = trademark.tmId + ':' + trademark.tmName
  getData(searchParams.value)
}

const attrInfos = (attr: AttrsList, attrValue: string) => {
  let prop = `${attr.attrId}:${attrValue}:${attr.attrName}`
  if (searchParams.value.props?.indexOf(prop) == -1) {
    searchParams.value.props?.push(prop)
  }
  getData(searchParams.value)
}
...
watch([route], (value, oldValue, onCleanup) => {
  ...
  searchParams.value.category2Id = undefined
  searchParams.value.category3Id = undefined
  Object.assign(searchParams.value, value[0].query, value[0].params)
  getData(searchParams.value)
